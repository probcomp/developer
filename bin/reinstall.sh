...
function installProbcompDocker {
  ...
  ask "use edge packages?" && \
    docker-compose -p ${USER} exec notebook bash -c "wget -O /tmp/conda_probcomp.txt --quiet $PROBCOMP_EDGE_PACKAGES_URL && \
        sudo -E -u jovyan conda install -n python2 --quiet --yes -c probcomp/label/edge -c cidermole -c fritzo -c ursusest \
        --file /tmp/conda_probcomp.txt && \
        sudo -E -u jovyan conda remove -n python2 --quiet --yes --force qt pyqt && \
        sudo -E -u jovyan fix-permissions $CONDA_DIR" && \
        return 0

    docker-compose -p ${USER} exec notebook bash -c "wget -O /tmp/conda_probcomp.txt --quiet $PROBCOMP_PACKAGES_URL && \
        sudo -E -u jovyan conda install -n python2 --quiet --yes -c probcomp -c cidermole -c fritzo -c ursusest \
        --file /tmp/conda_probcomp.txt && \
        sudo -E -u jovyan conda remove -n python2 --quiet --yes --force qt pyqt && \
        sudo -E -u jovyan fix-permissions $CONDA_DIR"
}
